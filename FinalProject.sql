...
/*Для виконання фінального проекту по MySQL  було представлено навчальну базу даних «employees»,  що містить в собі наступні таблиці:
1.	`departments`(Columns: dept_no, dept_name)
2.	`dept_emp`(Columns: emp_no, dept_no, from_date, to_date)
3.	`dept_manager`(Columns: emp_no, dept_no, from_date, to_date)
4.	`employees`(Columns: emp_no, birth_date, first_name, last_name, gender, hire_date)
